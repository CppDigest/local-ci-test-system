...
def _write_patched_workflow(
            ...
            for j in range(i + 1, len(lines)):
                ...
                if re.match(r"^\s+container\s*:\s*$", row):
                    ...
                    if not options_found:
                        ...
                        lines.insert(
                            j + 1,
                            f'{options_indent}options: "{container_mount_options}"\n',
                        )
                    break
            break
    ...
    for i, line in enumerate(lines):
        if re.match(r"^\s+-\s+name:\s+Patch Boost", line):
            already_patched = any(
                "capy-file-stats" in lines[j] for j in range(max(0, i - 15), i)
            )
    ...
    if image_tag:
        ...
        list_item_indent = lines[block_start][
    ...
    for i, line in enumerate(lines):
        if "https://codecov.io/bash" in line and "curl" in line:
            ...
            if stripped.strip().startswith("bash <(curl") or "bash <(curl" in stripped:
                ...
                lines[i] = (
                    f'{indent}{act_check}{rest}; else echo "Skipping Codecov upload (running under act)."; fi\n'
                )
            break
